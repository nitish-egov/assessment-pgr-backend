...
//    validateDepartment(request, mdmsData);
    ...
    RequestSearchCriteria criteria = RequestSearchCriteria.builder().ids(Collections.singleton(id))
        .build();
    ...
    if (CollectionUtils.isEmpty(serviceWrappers)) {
      throw new CustomException("INVALID_UPDATE",
          "The record that you are trying to update does not exists");
    }
  ...
  private void validateDepartment(ServiceRequest request, Object mdmsData) {
    ...
    if (CollectionUtils.isEmpty(res)) {
      throw new CustomException("PARSING_ERROR",
          "Failed to fetch department from mdms data for serviceCode: " + serviceCode);
    } else {
    ...
    if (!departments.contains(departmentFromMDMS)) {
      errorMap.put("INVALID_ASSIGNMENT",
          "The application cannot be assigned to employee of department: "
